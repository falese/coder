...
export async function runMlxTrain(
  ...
  let stdoutOutput = "";
  const recentLosses: number[] = []; // rolling window for loss-spike divergence detection
  ...
  for (;;) {
    ...
    for (const line of lines) {
      ...
      if (parsed) {
        ...
        // Detect divergence — accumulate the last few losses, but only check
        // after iter 20 so early warm-up noise never trips the abort.
